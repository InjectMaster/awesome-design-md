...
// gradient, button). Deliberately free of the chart engine so those items
// install without `core` — only palette.ts is shared. The Bayer matrix and
// bloom presets mirror dither-paint.ts so everything reads as one texture.
